...
class Text
{
	...
	public function clear($string)
	{
		...
		$string = mb_convert_encoding($string, 'UTF-8', mb_detect_encoding($string, 'UTF-8, ISO-8859-7', true));
		// Decode html entities
	 ...
	 * Sentence filtering
	 * - Upper case letters
	 * - Remove all kinds of modern greek language accents
	...
	public function process($value)
	{
		...
		// Remove accents
		$value = str_replace(array('Ά', 'Έ', 'Ή', 'Ί', 'Ϊ', 'Ό', 'Ύ', 'Ϋ', 'Ώ', 'ΐ', 'ΰ'), array('Α', 'Ε', 'Η', 'Ι', 'Ι', 'Ο', 'Υ', 'Υ', 'Ω', 'Ι', 'Υ'), $value);
		// Remove dots from abbreviations
		$value = str_replace('.', '', $value);
		// Remove words-breaking over new lines
		$value = str_replace(array("-\n", "\r"), '', $value);
		// Remove any non alphabetic characters
		$value = preg_replace('/[^ΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩ]/u', ' ', $value);
